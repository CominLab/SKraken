...
if [ -n "$KRAKEN_SCORE_FILTER" ] && [ "$KRAKEN_SCORE_FILTER" -ne 0 ]
then
  if [ -e "filtered.complete" ]
  then
    echo "Skipping additional step, DB already filtered."
  else
    echo "Filtrating DB with Score $KRAKEN_SCORE_FILTER (Additional step)..."
    start_time1=$(date "+%s.%N")
    find library/ '(' -name '*.fna' -o -name '*.fa' -o -name '*.ffn' ')' -print0 | \
      xargs -0 cat | \
      score_filter -S $KRAKEN_SCORE_FILTER -x -d database.kdb -i database.idx -l $KRAKEN_MINIMIZER_LEN\
      -n taxonomy/nodes.dmp -m seqid2taxid.map -F /dev/fd/0 -o database_red.kdb -r database_red.idx

    if [ -e "database_red.idx" ]
    then
      mv database.kdb database_full.kdb
      mv database.idx database_full.idx
      mv database_red.kdb database.kdb
      mv database_red.idx database.idx
      touch "filtered.complete"
    fi
    echo "Database filtered. [$(report_time_elapsed $start_time1)]"
  fi
else
  echo "Skipping additional step, no database filtering requested."
fi

...
std::pair<uint32_t *, int64_t> KrakenDB::kmer_query_pair(uint64_t kmer, uint64_t *last_bin_key,
                               int64_t *min_pos, int64_t *max_pos,
                               bool retry_on_failure)
{
  int64_t min, max, mid;
  ...
  while (min + 15 <= max) {
    ...
    if (kmer > comp_kmer)
      ...
      std::pair<uint32_t *, int64_t> ret_pair((uint32_t *) (ptr + pair_sz * mid + key_len), mid);
      return ret_pair;
    }
  }
  // Linear search once window shrinks
  ...
  std::pair<uint32_t *, int64_t> answer (NULL ,-1);
  // ROF implies the provided values might be out of date
  // If they are, we'll update them and search again
  if (retry_on_failure) {
    b_key = bin_key(kmer);
    ...
    max = index_ptr->at(b_key + 1) - 1;
    // Recursive call w/ adjusted search params and w/o retry
    answer = kmer_query_pair(kmer, &b_key, &min, &max, false);
    // Update caller's search params due to bin key change
    if (last_bin_key != NULL) {
      *last_bin_key = b_key;
      *min_pos = min;
...
// Binary search w/in the k-mer's bin
std::pair<uint32_t *, int64_t> KrakenDB::kmer_query_pair(uint64_t kmer) {
  return kmer_query_pair(kmer, NULL, NULL, NULL, false);
}

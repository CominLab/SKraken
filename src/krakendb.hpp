...
namespace kraken {
  ...
  class KrakenDB {
    ...
    std::pair<uint32_t *, int64_t> kmer_query_pair(uint64_t kmer);  // return ptr to pair w/ kmer

    // perform search over last range to speed up queries
    std::pair<uint32_t *, int64_t> kmer_query_pair(uint64_t kmer, uint64_t *last_bin_key,
                          int64_t *min_pos, int64_t *max_pos,
                          bool retry_on_failure=true);

...
namespace kraken {
  ...
  std::map<uint32_t, uint8_t> build_rank_map(string filename) {
    map<uint32_t, uint8_t> rmap;
    uint32_t node_id, parent_id;
    char rank [3];
    string line;
    ...
    while (ifs.good()) {
      ...
      if (line.empty())
        break;
      sscanf(line.c_str(), "%d\t|\t%d\t|\t%3s", &node_id, &parent_id, rank);
      rmap[node_id] = str_to_rank(rank);
    }
    rmap[1] = 0;
    return rmap;
  }
  ...
  std::multimap<uint32_t, uint32_t> build_children_multimap(std::map<uint32_t, uint32_t> pruned_parent_map) {
    multimap<uint32_t, uint32_t> cmmap;
    map<uint32_t,uint32_t>::iterator it;
    for( it = pruned_parent_map.begin(); it != pruned_parent_map.end(); ++it) {
      cmmap.insert ( std::pair<uint32_t,uint32_t> (it->second, it->first));
    }
    return cmmap;
  }
